...
<? include ('/opt/bitnami/apache2/htdocs/t/template/map_include.php'); ?>

<!-- HTML 시작 -->

<? include ('/opt/bitnami/apache2/htdocs/t/template/map_head.php'); ?>
<? include ('/opt/bitnami/apache2/htdocs/t/template/t_header.php'); ?>
<? include ('/opt/bitnami/apache2/htdocs/t/template/connect_information.php'); ?>
<?
  $dt = date("Ymd");
  $td = "${grade}_stud$dt";
  switch($room){
    case 'don' :  $sql = "SELECT * from $td WHERE first='동아리실1' or first='동아리실2'"; break;
    case 'won' :  $sql = "SELECT * from $td WHERE first='원탁1' or first='원탁2'"; break;
    case 'mul' :
        if($grade==1)   $sql = "SELECT * from $td WHERE mul='1멀' or mul='2멀'"; break;
        else if($grade==2 && $cha==1)   $sql = "SELECT * from $td WHERE first='멀티실'"; break;
        else if($grade==2 && $cha==2)   $sql = "SELECT * from $td WHERE second='멀티실'"; break;
    case 'etc' :
      if($cha==1){
          if($grade==1)
              $sql = "SELECT * from $td WHERE mul <>'1멀' and first <> '정독실' and first <> '동아리실1'
              and first <> '동아리실2' and first <> '원탁1' and first <> '원탁2' and first <> '외출' and first <> '귀가'
              and first <> '사물함' and first <> 'X' and first <> NULL"; break;
          else if($grade==2)
              $sql = "SELECT * from $td WHERE first <> '멀티실' and first <> '정독실' and first <> '동아리실1'
              and first <> '동아리실2' and first <> '원탁1' and first <> '원탁2' and first <> '외출' and first <> '귀가'
              and first <> '사물함' and first <> 'X' and first <> NULL"; break;
          else if($grade==3)
              $sql = "SELECT * from $td WHERE first <> '멀티실' and first <> '정독실' and first <> '외출' and first <> '귀가'
              and first <> 'X' and first <> NULL"; break;
      }else if($cha==2){
          if($grade==1)
              $sql = "SELECT * from $td WHERE mul <>'2멀' and second <> '정독실' and second <> '외출' and second <> '귀가'
              and first <> 'X' and first <> NULL"; break;
          else if($grade==2)
              $sql = "SELECT * from $td WHERE second <> '멀티실' and second <> '정독실' and second <> '외출' and second <> '귀가'
              and first <> 'X' and first <> NULL"; break;
          else if($grade==3)
              $sql = "SELECT * from $td WHERE second <> '멀티실' and second <> '정독실' and second <> '외출' and second <> '귀가'
              and first <> 'X' and first <> NULL"; break;
      }
    case 'sam' :  $sql = "SELECT * from $td WHERE first='사물함'"; break;
  }
  ...
  /*
  $s1 = "SELECT * from $td WHERE first='동아리실1' or first='동아리실2'";
  $s2 = "SELECT * from $td WHERE first='동아리실1' and one='1'";
  $s21 = "SELECT * from $td WHERE first='동아리실2' and one='1'";
  $s3 = "SELECT * from $td WHERE first='동아리실1' and one='0'";
  $s31 = "SELECT * from $td WHERE first='동아리실2' and one='0'";
